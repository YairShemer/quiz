...
st.write(nice_text("כל רמז מוריד 10 נקודות (לחץ על הרמזים שברצונך לחשוף)"), unsafe_allow_html=True)
score = 100
...
col1.empty()
score = hint(hint_title='הודעה ששלחתי לך בלינקאדין', hint_text='הי צביקה, אשמח לשמור על קשר (:',col=col1, score=score, operation=operation)

# score = hint(hint_title='מספר החדר שלך פחות מספר החדר שלי:', hint_text='',col=col1, score=score, operation = operation)
score = hint(hint_title='וצאפ ששלחת אלי', hint_text='בפאטיו בצד ימין, מחכים רק לך',col=col2, score=score, operation = operation)
col1.empty()
score = hint(hint_title='וצאפ ששלחתי אליך', hint_text='ערערתי וקיבלתי אישור לא להיכנס לבידוד (:',col=col2, score=score, operation = operation)
col1.empty()
